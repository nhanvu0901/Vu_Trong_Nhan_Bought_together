     ...
     <div  class="table-container">
        <table>
           <tr class="table-col-name" >
            <td><input type="checkbox" v-model="selectAllRecommend" @click="SelectAllRecommend" /></td>
            <td>Image</td>
            <td>Product Name</td>
             <td>Price</td>
           ...
           <tr v-for="(item,index) in filteredItemsRecommend" :key="item.product_id" class="table-row"  :class="{disabled:checked1===false}">
              <td>
                <input type="checkbox" @change='updateCheckallRecommend()' :id="item.product_id" v-model="selected_recommend[index]" :true-value="{item:item.product_name,id:item.product_id,item_img:item.product_img,item_price:item.product_price,item_compare:item.product_compare_at_price}"  :disabled="checked1 === false"/>
              </td>
              <td>
              <img :src="item.product_img" alt="" style="width: 28px;height: 30px;" :class="{disabled:checked1===false}">
              </td>
...
">
     ...
     <div  class="table-container">
        <table>
           <tr class="table-col-name">
             <td><input type="checkbox" v-model="selectAllExclude" @click="SelectAllExclude" /></td>
            <td>Image</td>
            <td>Product Name</td>
             <td>Price</td>
             <td>Compare At Price</td>
           ...
           <tr v-for="(item,index) in filteredItemsExclude" :key="item.product_id" class="table-row" :class="{disabled:checked1===false}">
              <td>
                <input type="checkbox" @change='updateCheckallExclude()' :id="item.product_id" v-model="selected_excluded[index]"  :true-value="{item:item.product_name,id:item.product_id,item_img:item.product_img,item_price:item.product_price,item_compare:item.product_compare_at_price}" :disabled="checked1 === false"/>
              </td>
              <td><img :src="item.product_img" alt="" style="width: 28px;height: 30px;"></td>
              <td>{{ item.product_name }}</td>
              <td>{{ item.product_price }}</td>
             <td>{{ item.product_compare_at_price }}</td>
             <td>{{ item.quantity }}</td>
  ...
  data() {
   ...
   methods: {
    SelectAllExclude(){
      ...
      this.selected_excluded = [];
      if (!this.selectAllExclude) {
          for (let i in this.filteredItemsExclude) {
            let item_data = JSON.parse(JSON.stringify(this.filteredItemsExclude[i]))
            let data ={
              id: item_data.product_id,
              item: item_data.product_name,
              item_compare: item_data.product_compare_at_price,
              item_img: item_data.product_img,
              item_price:item_data.product_price,
            }
              this.selected_excluded.push(data);
          }
      }

    },
   updateCheckallExclude : function(){
       if(this.filteredItemsExclude.length === this.selected_excluded.length){
                 this.selectAllExclude= true;
              }else{
                 this.selectAllExclude = false;
              }
     },
    SelectAllRecommend() {

      this.selected_recommend = [];
      if (!this.selectAllRecommend) {
          for (let i in this.filteredItemsRecommend) {
            let item_data = JSON.parse(JSON.stringify(this.filteredItemsRecommend[i]))
            let data ={
              id: item_data.product_id,
              item: item_data.product_name,
              item_compare: item_data.product_compare_at_price,
              ...
              this.selected_recommend.push(data);
          }
      }

    },
     updateCheckallRecommend : function(){
      console.log("Jelloo")
       let count = 0
       for (let i in this.selected_recommend){
         let item_data = JSON.parse(JSON.stringify(this.selected_recommend[i]))
         if(item_data !== false){
           count++
         }

       }
       if(this.filteredItemsRecommend.length === count){
                 this.selectAllRecommend = true;
      }else{
         this.selectAllRecommend = false;
      }
     },
