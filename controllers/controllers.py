...
class BoughtTogether(http.Controller):
    ...
    def redirect(self, **kw):
        ...
        if request.env.user and request.env.user.id == shopify_app_exist.user:
...
<body><h1>404</h1><p>Oops! Something is wrong.</p><a class="button" href="#"><i class="icon-home"></i> Go back in initial page, is better.</a></body></html>"""
    ...
    def authentocate_user(self, **kw):
        ...
        #     current_user_exist.write({
        #         "name":name,
        #         "login":shop_url
        #     })
        # else:
        #     letters = string.ascii_lowercase
        #     current_company = request.env['res.company'].sudo().search([('name', '=', shop_url)], limit=1)
        #     password_generate = ''.join(random.choice(letters) for i in range(30))
        #     if not current_company:
        #         currency_id = request.env['res.currency'].sudo().search(
        #             ['&', '|', ('active', '=', False), ('active', '=', True), ('name', '=', shopify_exist.currency)], limit=1)
        #         current_company = request.env['res.company'].sudo().create({
        #             'name':shop_url,
        #             'currency_id': currency_id.id
        #         })
        #     current_user_exist.create({
        #         "name": name,
        #         "login": shop_url,
