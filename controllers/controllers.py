    ...
    def redirect(self, **kw):
        ...
        if not scripTag:
             shopify.ScriptTag.create({
                "event": "onload",
                "src": web_base_url + "/bought_together/static/scriptag/" + "scriptag.js" + "?v=" + str(time.time())
            })
        else:
        ...
        if access_token:
            ...
            print("*******************")
            ngrok_url = 'https://cb65-27-72-103-14.ap.ngrok.io'



            webhook_products_create = shopify.Webhook()
            webhook_products_create.topic = "products/create"
            webhook_products_create.address = ngrok_url + "/webhook/products_create/" + str(shop.id)
            ...
            webhook_products_update.address = ngrok_url + "/webhook/products_update/" + str(shop.id)
